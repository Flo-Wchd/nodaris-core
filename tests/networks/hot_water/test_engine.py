...
def _pipe_catalog() -> PipeCatalog:
    ...
    return PipeCatalog(
        materials_by_code={"ECS": material},
        sizes_by_code={"P20": size},
        size_codes_by_material={"ECS": ["P20"]},
    )
...
def test_hot_water_facade_exposes_hot_water_side() -> None:
    nodes, sections = _network()

    engine = HotWaterNetworkEngine(
        nodes=nodes,
        sections=sections,
    ...
    assert engine.side is DomesticWaterSide.HOT_WATER
    assert engine.domestic_engine().side is DomesticWaterSide.HOT_WATER


def test_hot_water_facade_compute_sections_uses_hot_water_flow() -> None:
    nodes, sections = _network()
    ...
    sizing = result.value.section_results["S1"].sizing
...
def test_compute_hot_water_network_functional_entry_point() -> None:
    nodes, sections = _network()

    result = compute_hot_water_network(
        nodes=nodes,
        sections=sections,
        appliance_catalog=_appliance_catalog(),
        pipe_catalog=_pipe_catalog(),
